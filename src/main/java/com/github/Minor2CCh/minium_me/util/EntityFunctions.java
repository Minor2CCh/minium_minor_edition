...
public class EntityFunctions {
    public static final List<EquipmentSlot> EQUIPMENT_SLOT_LIST = List.of(EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET);
    ...
    public static boolean equipMiniumArmors(LivingEntity entity, int requireNum){
        int count = 0;
        List<ItemStack> equipList = getArmorList(entity);
    ...
    public static int numReinforcedComponent(LivingEntity entity, ArmorReinforcedComponent reinforced){
        int count = 0;
        List<ItemStack> equipList = getArmorList(entity);
        for(ItemStack stack : equipList){
            ArmorReinforcedComponent component = stack.get(MiniumModComponent.ARMOR_REINFORCED);
            if(component != null && component.equals(reinforced)){
                count++;
            }
        }
        return count;
    ...
    public static List<ItemStack> getArmorList(LivingEntity entity){
